...
class EyeGazeInference:
    ...
    def set_model(self):
        model_backbone = backbone.build_social_eye(self.config,)
        head_in_channel = model_backbone.out_channels
        head_out_channel = 2
        final_height_width = tuple(self.config["MODEL"]["HEAD"]["final_height_width"])

        head = SocialEyePredictionBoundHead(head_in_channel, head_out_channel, final_height_width)
    ...
    def post_process(self, preds):
        ...
        processed_preds = preds * torch.tensor(stats["sA"], device=self.device) + torch.tensor(stats["mA"], device=self.device)
